...
export function CounterPickTab() {
  ...
  const rows = useMemo(() => {
    return data.filter((row) => row.totalGames >= minGames);
  }, [data, minGames]);
              ...
              return (
                <article key={hero.hero} className="space-y-3 p-4">
                  <div className="flex items-center gap-3">
                    ...
                    >
                      기본 승률 {hero.baseWinRate.toFixed(1)}%
                    </span>
                  </div>
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <Image
                              src={HeroImage[counter.opponentHero]}
                              alt={counter.opponentHeroName}
                              width={36}
                              height={36}
                            ...
                            <span className="font-semibold text-rose-100 w-12 sm:w-12 md:w-24 lg:w-28 text-ellipsis overflow-hidden whitespace-nowrap">
                              {counter.opponentHeroName}
                            </span>
                          </div>
                          <div className="text-right pr-2">
                            <p className="text-xs text-gray-200">
                              상대전적: {counter.wins}승 {counter.losses}패 ({counter.games}경기)
                            </p>
                            <p className="text-sm font-bold text-rose-100">
                              승률 하락: -{counter.dropPercentPoint.toFixed(1)}%p
                            </p>
                          </div>
                        </div>
                      </div>
