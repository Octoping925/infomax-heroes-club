...
describe("buildTeammateRows", () => {
  it("선택 플레이어의 동팀 빈도를 비율과 횟수 순으로 정렬한다", () => {
    ...
    const emptyRecentStats = { encounterMatches: 0, sameTeamMatches: 0, sameTeamRate: 0 };
    const pairs = [
      {
        playerAId: "p1",
        playerBId: "p2",
        allTime: { encounterMatches: 10, sameTeamMatches: 6, sameTeamRate: 60 },
        recent6: emptyRecentStats,
      },
      {
        playerAId: "p3",
        playerBId: "p1",
        allTime: { encounterMatches: 5, sameTeamMatches: 3, sameTeamRate: 60 },
        recent6: emptyRecentStats,
      },
      {
        playerAId: "p2",
        playerBId: "p3",
        allTime: { encounterMatches: 4, sameTeamMatches: 4, sameTeamRate: 100 },
        recent6: emptyRecentStats,
      },
    ];
